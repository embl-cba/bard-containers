...
# Get current user (who starts the container)
CURRENT_USER=$(whoami)
...
if [[ -n "$HOSTNAME" ]]; then
    ...
    SLURM_HOSTNAME=$(cat /etc/hostname)
else
    SLURM_HOSTNAME=$(hostname)
fi

echo "Setting SlurmctldHost and NodeHostName to: $SLURM_HOSTNAME"

# Modify slurm.conf dynamically
sed -i "s/^SlurmctldHost=.*/SlurmctldHost=$SLURM_HOSTNAME/" /etc/slurm/slurm.conf
sed -i "s/^NodeName=.* NodeHostName=.*/& NodeHostName=$SLURM_HOSTNAME/" /etc/slurm/slurm.conf
...
then
    ...
    mysqladmin -uroot shutdown || killall mysqld || true
    #killall mysqld || true
