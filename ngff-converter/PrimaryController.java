...
public class PrimaryController {
    ...
    private void programHelp() {
	    ...
	    new ProcessBuilder("xdg-open", "https://github.com/glencoesoftware/NGFF-Converter?tab=readme-ov-file#usage").start();
        } catch (IOException e) {
    ...
    private void b2rHelp() {
	    ...
	    new ProcessBuilder("xdg-open", "https://ngff.openmicroscopy.org/").start();
        } catch (IOException e) {
            LOGGER.error("Failed to open URL");
            throw new RuntimeException(e);
        }
    ...
    private void r2oHelp() {
	    ...
	    new ProcessBuilder("xdg-open", "https://ome-model.readthedocs.io/en/stable/ome-tiff/").start();
        } catch (IOException e) {
            LOGGER.error("Failed to open URL");
            throw new RuntimeException(e);
        }
	...
	try{
            ...
            if (taskbar.isSupported(Taskbar.Feature.PROGRESS_VALUE)) {
                if (newProgress == 0) taskbar.setProgressValue(-1);
                else taskbar.setProgressValue((int) (newProgress * 100));
            }
    ...
    public void showExceptionDialog(Throwable ex) {
		...
		new ProcessBuilder("xdg-open", "https://github.com/glencoesoftware/NGFF-Converter/issues").start();
            } catch (IOException e) {
                LOGGER.error("Failed to open URL");
            }
        });
